...
var Gamma = (function() {
	...
	var $window = $( window ),
		...
		defaults = {
			...
			speed : 150,
			easing : 'cubic-bezier(0.84, .01, 0.19, .93)',
			...
			svImageTransitionEasingFade : 'cubic-bezier(0.84, .01, 0.19, .93)',
			svImageTransitionSpeedResize : 100,
			svImageTransitionEasingResize : 'cubic-bezier(0.84, .01, 0.19, .93)',
			svMarginsVH : {
				vertical : 140,
				horizontal : 120
			},
		...
		_createSingleViewNavigation = function() {
			...
			if( Gamma.itemsCount > 1 ) {
				...
				Gamma.nav = $( '<nav class="gamma-nav"><span class="icon-left"></span><span class="icon-right"></span></nav>' ).appendTo( Gamma.singleview );
				Gamma.svnavnext = Gamma.nav.find( 'span.icon-right' );
				Gamma.svnavprev = Gamma.nav.find( 'span.icon-left' );
